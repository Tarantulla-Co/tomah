...
export const mockApi = {
  ...
  async createOrder(body:OrderCreateRequest):Promise<OrderCreateResponse>{
    ...
    const orderNumber=`TOM-MOCK-${token}`;
    return {orderNumber,status:'PENDING_PAYMENT',amounts:{subtotal:decimal(subtotal),shipping:decimal(shipping),tax:decimal(tax),total:decimal(total),currency:body.currency},payment:{provider:'manual',online:false,reference:orderNumber,publicKey:null,clientSecret:null,authorizationUrl:null,devConfirmPath:`/public/checkout/${orderNumber}/confirm-dev`}};
  },
